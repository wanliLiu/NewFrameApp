...
public class Luban {
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = FileUtil.INSTANCE.getPicUploadTempPath(ctx, file.getAbsolutePath());

        if (new File(thumb).exists()) {
            MLog.d("图片压缩", "缓存已经有，不需要再压缩" + ImageDeal.INSTANCE.getDebugString(thumb));
    ...
    private File firstCompress(@NonNull File file) {
        ...
        String thumbFilePath = FileUtil.INSTANCE.getPicUploadTempPath(ctx, file.getAbsolutePath());

        if (new File(thumbFilePath).exists()) {
            MLog.d("图片压缩", "缓存已经有，不需要再压缩" + ImageDeal.INSTANCE.getDebugString(thumbFilePath));
